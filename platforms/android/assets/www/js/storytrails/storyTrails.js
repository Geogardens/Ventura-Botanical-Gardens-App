...
function appMap() {
    ...
    this.imageryService = "http://tiles.arcgis.com/tiles/UdNSRpyD8hxLx3jb/arcgis/rest/services/imagery/MapServer";
    this.gardenService = "http://tiles.arcgis.com/tiles/UdNSRpyD8hxLx3jb/arcgis/rest/services/VBG_Garden/MapServer";
    ...
    $.when($.ajax({
        ...
        for (var i = 0; i < config.features.length; i++) {
            ...
            switch (key.toLowerCase()) {
                case "imageryservice":
                    stApp.map.imageryService = value;
                    break;
                case "gardenservice":
...
function loadStops(tour) {
    return $.ajax({
        ...
        async:false,
        data: {
            where: "Not Upper(Visible) in ('F','N','FALSE','NO','0') or Visible is null",
            f: "json",
